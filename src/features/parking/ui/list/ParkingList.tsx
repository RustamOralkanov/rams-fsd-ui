    ...
    return (
        <Flex vertical gap={40} className="parking-list">
            ...
            <Flex vertical className="border-gray-100 border-radius-l padding-30" style={{ aspectRatio: "12.4 / 7.37" }}>
                ...
                <svg width="100%" height="100%" viewBox={`0 0 ${parkings.parkings["-1"].width} ${parkings.parkings["-1"].height}`}>
                    <image href={"https://ramsqz.com" + parkings.parkings["-1"].image} />
                    {Object.values(parkings.parkings["-1"]).map((parking, index) => {
                        if (typeof parking === "object" && parking !== null && "parking_plan" in parking && parking.parking_plan?.coordinates) {
                            ...
                            const content = (
                                <Flex vertical gap={20} className="padding-20 border-radius-m">
                                    <Flex align="center" justify="space-between" gap={45}>
                                        <Title level={4}>Машино-место</Title>
